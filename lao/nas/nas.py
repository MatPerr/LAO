...
from lao.embedding.autoencoder import ArcAE, CosineAnnealingAlphaLR, get_device
...
class LogPiExpectedImprovement(AnalyticAcquisitionFunction):
    ...
    def __init__(
        ...
        Args:
            ...
            best_f (Any): Input parameter.
            posterior_transform (Any): Input parameter.
            pi_func (Any): Input parameter.
            beta (Any): Input parameter.
            n (Any): Input parameter.
            maximize (Any): Input parameter.
            bounds (Any): Input parameter.
        """
        ...
        self.beta = beta
        self.n = n
        self.log_ei = LogExpectedImprovement(model=model, best_f=best_f, maximize=maximize)
...
def standardize(Y: Any, return_moments: Any = False) -> Any:
    """
    Standardize.

    Args:
        Y (Any): Input parameter.
        return_moments (Any): Input parameter.
...
def ZQL(x: Any, x1: Any, x2: Any, m: Any) -> Any:
    """
    Zql.

    Args:
        x (Any): Input parameter.
        x1 (Any): Input parameter.
        x2 (Any): Input parameter.
...
class LSBO_problem:
    ...
    def __init__(
        self,
        trained_ae: Any,
        cost_function: Any = "accuracy",
        dataset: Any = "cifar10",
        input_shape: Any = [3, 32],
        num_classes: Any = None,
        custom_cost_fn: Any = None,
        log_dir: Any = "runs/nas",
        acquisition_type: Any = "logEI",
        pi_func: Any = None,
    ) -> None:
        """
        Init.
    ...
    def _prepare_dataset(self) -> Any:
        ...
        test_loader = MultiEpochsDataLoader(test_dataset, batch_size=128, shuffle=False, num_workers=4, pin_memory=True)
        return (train_loader, test_loader)

    def _train_and_eval_model(self, model: Any, max_iterations: Any = 20000, model_idx: Any = None) -> Any:
        """
        Train and eval model.

        Args:
            model (Any): Input parameter.
            max_iterations (Any): Input parameter.
            model_idx (Any): Input parameter.
        ...
        while iteration < max_iterations:
            for x, y in train_loader:
                ...
                pbar.set_description(
                    f" n_iter {iteration + 1} | loss {loss.item():.2e} | acc {100 * acc:.2f} | lr {optimizer.param_groups[0]['lr']:.2e}"
                )
                pbar.update(1)
    ...
    def _validate_model(self, model: Any, test_loader: Any, criterion: Any) -> Any:
        """
        Validate model.

        Args:
            model (Any): Input parameter.
            test_loader (Any): Input parameter.
            criterion (Any): Input parameter.
    ...
    def _evaluate_architecture(self, z: Any, model_idx: Any = None) -> Any:
        """
        ...
        graph = ArcGraph(
            search_space=self.search_space, V=graph_vector[0], n_nodes=self.search_space.graph_features.n_nodes[0]
        )
        ...
        if self.cost_function == "params_x_FLOPs":
            ...
            print(
                f"n_params: {n_params:,} | FLOPs {FLOPs:,} | cost: {cost:.4f} | c_n_params: {c_params:.4f} | c_FLOPs: {c_FLOPs:.4f}"
            )
            if model_idx is not None:
                fig_path = os.path.join(self.save_dir, f"arch_{model_idx}.png")
                blueprint.plot(output_path=fig_path)
            ...
            if n_params > 1000000.0 or FLOPs > 100000000.0 or BBGP < 2:
                accuracy = np.float64(0.7)
                acc_var = np.float64(0.07**2)
            else:
                accuracy = np.float64(self._train_and_eval_model(model))
                acc_var = np.float64(0.003**2)
            cost = -accuracy + ZQL(n_params, 200000, 250000, 0.1) + ZQL(FLOPs, 60000000, 80000000, 0.1)
            if model_idx is not None:
                torch.save(model.state_dict(), os.path.join(self.save_dir, f"model_{model_idx}.pth"))
                blueprint = graph.to_blueprint(input_shape=self.input_shape, num_classes=self.num_classes)
                fig_path = os.path.join(self.save_dir, f"arch_{model_idx}.png")
            ...
            result = self.custom_cost_fn(graph, input_shape=self.input_shape, num_classes=self.num_classes)
    ...
    def _initialize_gp(self, n_initial: Any = 10) -> Any:
        """
        Initialize gp.

        Args:
            n_initial (Any): Input parameter.
    ...
    def _update_gp(self, X: Any, Y: Any, Yvar: Any = None) -> Any:
        """
        Update gp.
        ...
        self.gp = SingleTaskGP(
            train_X=X_normalized, train_Y=Y_standardized, train_Yvar=train_Yvar, covar_module=covar_module
        )
        mll = ExactMarginalLogLikelihood(self.gp.likelihood, self.gp)
        fit_gpytorch_mll(mll)

    def _optimize_acquisition(self, iteration: Any = None) -> Any:
        """
        Optimize acquisition.

        Args:
            iteration (Any): Input parameter.
        ...
        if self.acquisition_type == "logEI":
            ...
            acq_func = LogExpectedImprovement(model=self.gp, best_f=Y_star, maximize=self.maximize)
        elif self.acquisition_type == "EI":
            Y_star = self.Y.max().item() if self.maximize else self.Y.min().item()
            acq_func = ExpectedImprovement(model=self.gp, best_f=Y_star, maximize=self.maximize)
        ...
        candidate, acq_value = optimize_acqf(
            ...
            bounds=torch.stack(
                [torch.zeros(self.z_dim, device=self.BO_device), torch.ones(self.z_dim, device=self.BO_device)]
            ),
            q=1,
            num_restarts=10,
            raw_samples=10000,
    ...
    def run(self, iterations: Any = 100, n_initial: Any = 10) -> Any:
        """
        Run.
    ...
    def visualize_best_architecture(self, z: Any = None, filename: Any = "best_architecture.png") -> Any:
        """
        Visualize best architecture.

        Args:
            z (Any): Input parameter.
            filename (Any): Input parameter.
    ...
    def evaluate_specific_architecture(self, z: Any) -> Any:
        """Evaluate specific architecture."""
        return self._evaluate_architecture(z)


def find_argmin(model: Any, input_shape: Any = (2,), lr: Any = 0.01, n_steps: Any = 2000, device: Any = "cpu") -> Any:
    """
    Find argmin.
    ...
    scheduler = CosineAnnealingAlphaLR(optimizer, T_max=n_steps, alpha=0.01)
...
if __name__ == "__main__":
    ...
    params_pi_func = nn.Sequential(
        ae.params_predictor,
        Lambda(lambda x: torch.maximum(torch.log(torch.Tensor([200000])), x)),
    ...
    FLOPs_pi_func = nn.Sequential(
        ae.FLOPs_predictor,
        Lambda(lambda x: torch.maximum(torch.log(torch.Tensor([60000000])), x)),
