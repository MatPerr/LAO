...
class CustomSE(nn.Module):
    def __init__(self, channels: Any, reduction: Any = 16) -> None:
        """
        Init.
...
class ChannelPad(nn.Module):
    ...
    def __init__(self, in_channels: Any, out_channels: Any, padding_mode: Any = "zero") -> None:
        """
        Init.
        ...
        Args:
            ...
            padding_mode (Any): Input parameter.
        """
        ...
        if padding_mode == "repeat" and out_channels % in_channels != 0:
            raise ValueError(
                f"For 'repeat' mode, out_channels ({out_channels}) must be divisible by in_channels ({in_channels})"
            )
...
class AggTensors(nn.Module):
    def __init__(self, aggregation: Any = "sum") -> None:
        """
        Init.
        ...
        if aggregation != "sum":
            raise NotImplementedError(
                f"Aggregation type {aggregation} not implemented, only 'sum' is supported for now"
            )
        self.aggregation = aggregation

    def forward(self, stacked_x: Any) -> Any:
        """Forward."""
